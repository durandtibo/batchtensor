...
Combine multiple operations for data processing:

```pycon
>>> import torch
>>> from batchtensor.nested import slice_along_batch, to
>>> # Original batch
>>> batch = {
...     "features": torch.tensor([[1, 2], [3, 4], [5, 6]], dtype=torch.int32),
...     "labels": torch.tensor([0, 1, 2]),
... }
>>> # Take first 2 samples and convert to float
>>> processed = to(slice_along_batch(batch, stop=2), dtype=torch.float32)
>>> processed
{'features': tensor([[1., 2.], [3., 4.]]), 'labels': tensor([0., 1.])}
...
Apply random transformations consistently:
...
>>> batch = {
...     "images": torch.randn(4, 3, 32, 32),  # 4 images
...     "labels": torch.tensor([0, 1, 2, 3]),
... }
>>> # Shuffle consistently across all tensors
>>> generator = get_torch_generator(42)
>>> shuffled = shuffle_along_batch(batch, generator=generator)
>>> # Both images and labels are shuffled with the same permutation
